/* prog3_participant.c - code for example client program that uses TCP */
...
int main( int argc, char **argv) {
	...
	if(connected == 'Y'){
		...
		while((length == 0 || length > 10) && valid == 'T' ){
			printf("Please enter the name in which you wish to be known as: ");
			...
			if(length < 10 && length > 0){
				printf("Trying to send username\n");
	...
	while(1){
		...
		uint16_t msgLength = strlen(buf);
		char msgBuf[msgLength];
		strncpy(msgBuf, buf, msgLength -1);
		uint16_t msgNetLength = htons(msgLength);
		send(sd, &msgNetLength, sizeof(uint16_t), 0);
		send(sd, msgBuf, msgLength, 0);
	}
	close(sd);
